...
def add_config(configs, asset_path, key_index, msg, locale):
    ...
    config_path = f"{path_split[0]}/config.{locale}.cfg"
    ...
    if asset_name not in configs[config_path]:
...
locale = sys.argv[1]
po_file = polib.pofile(f"{locale}.po")
...
for config_path in configs:
    with open(config_path, "w") as config_file:
        ...
        config_file.write(f"; Generated by inject_po.py at {creation_date_str}\n")

        for asset_name in configs[config_path]:
            config_file.write(f"[{asset_name}" + "]\n")
